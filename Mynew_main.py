...
parser.add_argument('--b', type=float, default=0.000001)
...
parser.add_argument('--usel2', action='store_true',default=False)
parser.add_argument('--useUnified', action='store_true',default=False)
parser.add_argument('--usexTime', action='store_true',default=False)
parser.add_argument('--usejTime', action='store_true',default=False)
parser.add_argument('--useTime', action='store_true',default=False)
parser.add_argument('--duibi', action='store_true',default=False)
parser.add_argument('--useOld', action='store_true',default=False)
parser.add_argument('--useMin', action='store_true',default=False)
...
if opt.record:
    log_file = f'results1/{opt.data}_ba_{opt.batchSize}_G_{opt.gpu}_dim_{opt.hidden_size}_UM_{opt.user_max_length}_IM_{opt.item_max_length}_K_{opt.k_hop}' \
               f'_layer_{opt.layer_num}_l2_{opt.l2}_usel2_{opt.usel2}_usexTime_{opt.usexTime}_usejTime_{opt.usejTime}_useTime_{opt.useTime}_duibi_{opt.duibi}_{opt.a}_compare_{opt.compare}_{opt.b}_topk_no_useOld_{opt.useOld}_seed_3407_useMin_{opt.useMin}'
...
if(opt.useOld==False):
    if(opt.useMin==False):
        train_root = f'Newdata/{opt.data}_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}_nol/train/'
        test_root = f'Newdata/{opt.data}_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}_mol/test/'
        val_root = f'Newdata/{opt.data}_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}_mol/val/'
    else:
        train_root = f'Newdata/{opt.data}_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}_min/train/'
        test_root = f'Newdata/{opt.data}_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}_min/test/'
        val_root = f'Newdata/{opt.data}_{opt.item_max_length}_{opt.user_max_length}_{opt.k_hop}_min/val/'
else:
...
stop_num = 0
torch.cuda.manual_seed(3407)#2022
for epoch in range(opt.epoch):
    ...
    for user, batch_graph, label, last_item in train_data:
        iter += 1
        score,users,items ,loss1= model(batch_graph.to(device), user.to(device), last_item, is_training=True)
        ...
        if(opt.usel2==True):
            ...
            loss+=1e-5*regularizer/users.shape[0]
    ...
    with torch.no_grad():
        for user, batch_graph, label, last_item, neg_tar in test_data:
            iter+=1
            score, top = model(batch_graph.to(device), user.to(device), last_item, neg_tar=torch.cat([label.unsqueeze(1), neg_tar],-1).to(device),  is_training=False)
